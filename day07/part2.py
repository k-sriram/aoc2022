...
def compute(s: str) -> int:
    ...
    return find_smallest_deletable_dir(fs)
...
def find_smallest_deletable_dir(fs: dict[tuple[str, ...], int]) -> int:
    total_size = 70000000
    required_size = 30000000
    used = fs[("/",)]
    free_up_size = required_size - total_size + used

    smallest = None
    for _, size in fs.items():
        if size >= free_up_size and (smallest is None or size < smallest):
            smallest = size
